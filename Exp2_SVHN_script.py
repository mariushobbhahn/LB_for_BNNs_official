...
def main():
    ...
    Diag_SODPP_NLL_in = []
    Diag_SODPP_NLL_CIFAR100 = []
    Diag_SODPP_NLL_CIFAR10 = []
    Diag_SODPP_ECE_in = []
    Diag_SODPP_ECE_CIFAR100 = []
    Diag_SODPP_ECE_CIFAR10 = []
    ...
    KFAC_SODPP_NLL_in = []
    KFAC_SODPP_NLL_CIFAR100 = []
    KFAC_SODPP_NLL_CIFAR10 = []
    KFAC_SODPP_ECE_in = []
    KFAC_SODPP_ECE_CIFAR100 = []
    KFAC_SODPP_ECE_CIFAR10 = []
    """
    ...
    for s in range(args.num_seeds):
        ...
        MAP_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_MAP)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        MAP_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_MAP)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        MAP_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_MAP)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        MAP_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_MAP))
        MAP_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_MAP))
        MAP_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_MAP))
        ...
        Diag_samples_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_D)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        Diag_samples_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_D)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        Diag_samples_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_D)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        Diag_samples_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_D))
        Diag_samples_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_D))
        Diag_samples_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_D))
        ...
        KFAC_samples_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_K)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        KFAC_samples_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_K)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        KFAC_samples_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_K)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        KFAC_samples_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_K))
        KFAC_samples_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_K))
        KFAC_samples_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_K))
        ...
        Diag_LB_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_LB_D)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        Diag_LB_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_LB_D)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        Diag_LB_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_LB_D)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        Diag_LB_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_LB_D))
        Diag_LB_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_LB_D))
        Diag_LB_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_LB_D))
        ...
        KFAC_LB_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_LB_KFAC)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        KFAC_LB_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_LB_KFAC)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        KFAC_LB_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_LB_KFAC)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        KFAC_LB_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_LB_KFAC))
        KFAC_LB_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_LB_KFAC))
        KFAC_LB_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_LB_KFAC))
        ...
        Diag_LB_norm_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_LB_Dn)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        Diag_LB_norm_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_LB_Dn)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        Diag_LB_norm_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_LB_Dn)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        Diag_LB_norm_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_LB_Dn))
        Diag_LB_norm_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_LB_Dn))
        Diag_LB_norm_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_LB_Dn))
        ...
        KFAC_LB_norm_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_LB_KFACn)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        KFAC_LB_norm_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_LB_KFACn)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        KFAC_LB_norm_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_LB_KFACn)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        KFAC_LB_norm_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_LB_KFACn))
        KFAC_LB_norm_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_LB_KFACn))
        KFAC_LB_norm_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_LB_KFACn))


        #PROBIT diag
        SVHN_test_in_PROBIT_D = predict_probit(la_diag, SVHN_test_loader, timing=False, device=device).cpu().numpy()
        SVHN_test_out_CIFAR100_PROBIT_D = predict_probit(la_diag, CIFAR100_test_loader, timing=False, device=device).cpu().numpy()
        SVHN_test_out_CIFAR10_PROBIT_D = predict_probit(la_diag, CIFAR10_test_loader, timing=False, device=device).cpu().numpy()
        ...
        Diag_PROBIT_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_PROBIT_D)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        Diag_PROBIT_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_PROBIT_D)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        Diag_PROBIT_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_PROBIT_D)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        Diag_PROBIT_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_PROBIT_D))
        Diag_PROBIT_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_PROBIT_D))
        Diag_PROBIT_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_PROBIT_D))

        
        #PROBIT KFAC
        SVHN_test_in_PROBIT_K = predict_probit(la_kron, SVHN_test_loader, timing=False, device=device).cpu().numpy()
        SVHN_test_out_CIFAR100_PROBIT_K = predict_probit(la_kron, CIFAR100_test_loader, timing=False, device=device).cpu().numpy()
        SVHN_test_out_CIFAR10_PROBIT_K = predict_probit(la_kron, CIFAR10_test_loader, timing=False, device=device).cpu().numpy()
        ...
        KFAC_PROBIT_NLL_in.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_in_PROBIT_K)).log_prob(torch.tensor(targets_SVHN)).mean().item())
        KFAC_PROBIT_NLL_CIFAR100.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR100_PROBIT_K)).log_prob(torch.tensor(targets_CIFAR100)).mean().item())
        KFAC_PROBIT_NLL_CIFAR10.append(-torch.distributions.Categorical(torch.tensor(SVHN_test_out_CIFAR10_PROBIT_K)).log_prob(torch.tensor(targets_CIFAR10)).mean().item())
        KFAC_PROBIT_ECE_in.append(scoring.expected_calibration_error(targets_SVHN, SVHN_test_in_PROBIT_K))
        KFAC_PROBIT_ECE_CIFAR100.append(scoring.expected_calibration_error(targets_CIFAR100, SVHN_test_out_CIFAR100_PROBIT_K))
        KFAC_PROBIT_ECE_CIFAR10.append(scoring.expected_calibration_error(targets_CIFAR10, SVHN_test_out_CIFAR10_PROBIT_K))
    ...
    results_dict = {
        ...
        'Diag_PROBIT_NLL_in':Diag_LB_norm_ECE_CIFAR10,
